...
const MODEL = {
  SCHEDULE: 'promptday_Q4_K_M.gguf'
}

let scheduleLlamaChatSession: LlamaChatSession | null = null
export const getScheduleLlamaChatSession = async () => {
  ...
  const scheduleModel = await llama.loadModel({ modelPath: scheduleModelPath })

  const scheduleModelContext = await scheduleModel.createContext()
  const scheduleModelSession = new LlamaChatSession({
    contextSequence: scheduleModelContext.getSequence()
...
// "${user_input}"
// `
export const getScheudleDataFromPrompt = async (prompt: string) => {
  // console.log({ prompt })
  if (!scheduleLlamaChatSession) {
    scheduleLlamaChatSession = await getScheduleLlamaChatSession()
  }
  // console.log(scheduleLlamaChatSession)
  try {
    const result = await scheduleLlamaChatSession.prompt(prompt)
    // [Error] load: special_eos_id is not in special_eog_ids - the tokenizer config may be incorrect
    console.log(result)
    return result
  } catch (err) {
    logging.error(err)
